...
if [ -z "${HUDI_HIVE_SYNC_BUNDLE}" ]; then
  error_exit "Please make sure the environment variable HUDI_HIVE_SYNC_BUNDLE is setup"
fi

DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
#Ensure we pick the right jar even for hive11 builds
HUDI_HIVE_UBER_JAR=${HUDI_HIVE_SYNC_BUNDLE}
...
echo "Running Command : java -cp $HUDI_HIVE_SYNC_BUNDLE:${HADOOP_HIVE_JARS}:${HADOOP_CONF_DIR}:${HIVE_HOME}lib/* org.apache.hudi.hive.replication.HiveSyncGlobalCommitTool $@"
java -cp $HUDI_HIVE_SYNC_BUNDLE:${HADOOP_HIVE_JARS}:${HADOOP_CONF_DIR}:${HIVE_HOME}lib/* org.apache.hudi.hive.replication.HiveSyncGlobalCommitTool "$@"
